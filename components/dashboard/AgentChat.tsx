...
export default function AgentChat() {
  ...
  const send = async () => {
    ...
    const lastAgent = messages
      ...
      .reverse()
      .find((m) => m.role === "agent")
    const questionId = lastAgent ? `qa_${Buffer.from(lastAgent.text).toString("base64").slice(0, 10)}` : `qa_init`
    ...
    if (data.error) {
      setMessages((prev) => [...prev, { role: "agent", text: "Server error. Try again." }])
      return
    }

    if (data.done) {
      setMessages((prev) => [...prev, { role: "agent", text: "All information collected — saving..." }])
      // call save
      ...
      if (saveData.success) {
        setMessages((prev) => [...prev, { role: "agent", text: `Saved at prebuilds/${saveData.folder}` }])
      } else {
        setMessages((prev) => [...prev, { role: "agent", text: `Save failed: ${saveData.error || "unknown"}` }])
      }
      return
    }

    if (data.nextQuestion) {
      setMessages((prev) => [...prev, { role: "agent", text: data.nextQuestion.text }])
    } else {
      setMessages((prev) => [...prev, { role: "agent", text: "No next question, please try again." }])
    }
  }
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 w-96 flex flex-col bg-card border border-border rounded-lg shadow-xl overflow-hidden">
      <div className="bg-primary text-primary-foreground px-4 py-3 font-semibold text-sm">Agent Assistant</div>

      <div className="flex-1 overflow-y-auto p-4 space-y-3 max-h-80 bg-background">
        {messages.length === 0 ? (
          <div className="text-center text-muted-foreground text-sm py-8">Start a conversation...</div>
        ) : (
          <>
            {messages.map((m, i) => (
