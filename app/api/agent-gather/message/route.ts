...
export async function POST(req: Request) {
  try {
    ...
    if (!sessionId || !questionId) return NextResponse.json({ error: "missing" }, { status: 400 });

    if (!sessions[sessionId]) sessions[sessionId] = { answers: {}, history: [], system: "" };
    // save answer
    sessions[sessionId].answers[questionId] = answer;
    sessions[sessionId].history.push({ role: "user", content: `${questionId}: ${answer}` });

    // Build prompt for OpenAI: include system + current answers
    const systemPrompt = sessions[sessionId].system || "You are an agent that collects website requirements and dont repeat any question after you once asked strongly follow it.";
...
const res = await client.chat.completions.create({
    ...
    messages: [
      ...
      { role: "user", content: `New answer added: ${questionId} = ${answer}. Continue next question.` }
    ],
    temperature: 0.6,
    max_tokens: 100
  });
    ...
    if (text.toLowerCase().includes("done") || text.toLowerCase().includes("all set")) {
